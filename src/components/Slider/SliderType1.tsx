...
export default function SliderType1({ data }: SliderType1Props) {
  ...
  const init = (isIntro = false) => {
    ...
    const detailsActive = detailsEvenStateRef.current
    ...
    const progressTotalWidth =
      layoutMode === "mobile" || layoutMode === "landscape"
        ? 180
        : layoutMode === "tablet"
          ? 220
          : 500;
    ...
    rest.forEach((i, index) => {
      ...
      const stackOffset = isRTL
        ? window.innerWidth - ol - cw - index * (cw + gp)
        : ol + index * (cw + gp);
    ...
    if (isIntro) {
      ...
      const detailsActiveId = detailsEvenStateRef.current
        ? "details-even"
        : "details-odd";
  ...
  const stepForward = () => {
    return new Promise<void>((resolve) => {
      ...
      const detailsActive = newDetailsEven
        ? detailsEvenRef.current
        : detailsOddRef.current;
      const detailsInactive = newDetailsEven
        ? detailsOddRef.current
        : detailsEvenRef.current;
      ...
      const isSmall = layoutMode === "mobile" || layoutMode === "landscape";
      const progressTotalWidth = isSmall
        ? 180
        : layoutMode === "tablet"
          ? 220
          : 500;
      gsap.to(progressRef.current, {
        width: progressTotalWidth * (1 / newOrder.length) * (active + 1),
        ease,
      });
      ...
      gsap.to(cardsRef.current[active], {
        ...
        onComplete: () => {
          const xNew = isRTL
            ? window.innerWidth - ol - cw - (rest.length - 1) * (cw + gp)
            : ol + (rest.length - 1) * (cw + gp);
          ...
          const detailsInactiveId = newDetailsEven
            ? "details-odd"
            : "details-even";
  ...
  return (
    <div className="relative w-screen h-screen bg-[#1a1a1a] text-[#FFFFFFDD] overflow-hidden font-['Inter']">
      ...
      <div
        ref={indicatorRef}
        className="fixed left-0 right-0 top-0 h-[5px] z-[60] bg-[#ecad29]"
      />
      ...
      {data.map((item, index) => (
        ...
        <div
            ref={(el) => {
              cardContentsRef.current[index] = el;
            }}
            className={`absolute top-0 text-[#FFFFFFDD] pointer-events-none text-start ${isRTL ? 'left-3 pe-2' : 'left-0 ps-2'}`}
          >
            <div
              className={`w-[30px] h-[5px] rounded-full bg-[#FFFFFFDD] me-auto ${layoutMode === "mobile" || layoutMode === "landscape" ? "hidden" : ""}`}
            />
            <div
              className={`mt-1.5 font-medium ${layoutMode === "mobile" || layoutMode === "landscape" ? "text-[8px]" : "text-[13px]"}`}
            >
              {item.place}
            </div>
            <div
              className={`font-semibold font-['Oswald'] ${layoutMode === "mobile" || layoutMode === "landscape" ? "text-[11px]" : "text-[20px]"}`}
            >
              {item.title}
            </div>
            <div
      ...
      >
        <div className="h-[46px] overflow-hidden">
          <div className={`text pt-4 text-[20px] relative before:content-[''] before:absolute before:top-0 ${isRTL ? 'before:right-0' : 'before:left-0'} before:w-[30px] before:h-1 before:rounded-full before:bg-white`}>
            {data[textIndices.even].place}
          </div>
        </div>
        <div
          className={`mt-0.5 overflow-hidden ${layoutMode === "mobile" || layoutMode === "landscape" ? "h-[45px]" : "h-[100px]"}`}
        >
          <div
            className={`title-1 font-semibold font-['Oswald'] ${layoutMode === "mobile" || layoutMode === "landscape" ? "text-[32px]" : "text-[72px]"}`}
          >
            {data[textIndices.even].title}
          </div>
        </div>
        <div
          className={`mt-0.5 overflow-hidden ${layoutMode === "mobile" || layoutMode === "landscape" ? "h-[45px]" : "h-[100px]"}`}
        >
          <div
            className={`title-2 font-semibold font-['Oswald'] ${layoutMode === "mobile" || layoutMode === "landscape" ? "text-[32px]" : "text-[72px]"}`}
          >
            {data[textIndices.even].title2}
          </div>
        </div>
        <div
          className={`desc mt-4 ${layoutMode === "mobile" || layoutMode === "landscape" ? "w-[300px] text-xs" : "w-[500px]"}`}
        >
          {data[textIndices.even].description}
        </div>
        <div
          className={`cta mt-6 flex items-center gap-4 ${layoutMode === "mobile" || layoutMode === "landscape" ? "w-[300px]" : "w-[500px]"}`}
        >
          ...
          <button className="discover border border-white bg-transparent h-9 rounded-full text-white px-6 text-xs uppercase">
            Discover Location
          </button>
        </div>
      </div>
      ...
      >
        ...
        >
          <div
            className={`title-2 font-semibold font-['Oswald'] ${layoutMode === "mobile" || layoutMode === "landscape" ? "text-[32px]" : "text-[72px]"}`}
          >
            {data[textIndices.odd].title2}
          </div>
        </div>
        <div
          className={`desc mt-4 ${layoutMode === "mobile" || layoutMode === "landscape" ? "w-[300px] text-xs" : "w-[500px]"}`}
        >
          {data[textIndices.odd].description}
        </div>
        <div
          className={`cta mt-6 flex items-center gap-4 ${layoutMode === "mobile" || layoutMode === "landscape" ? "w-[300px]" : "w-[500px]"}`}
        >
      ...
      <div
        ref={paginationRef}
        className={`absolute right-0 top-0 inline-flex items-center w-full ${locale ==="en" ? "justify-start" : "justify-end"}`}
          style={{ display: layoutMode === "desktop" ? "flex" : "none" }}
      >
        <div
        ...
        >
          <svg
            ...
            viewBox="0 0 24 24"
            stroke="currentColor"
            className={`stroke-[2] text-[#ffffff99] ${layoutMode === "mobile" || layoutMode === "landscape" ? "w-4 h-4" : layoutMode === "tablet" ? "w-5 h-5" : "w-6 h-6"}`}
          >
            <path
              strokeLinecap="round"
        ...
        <div
          className={`z-[60] h-[50px] flex items-center ${layoutMode === "mobile" || layoutMode === "landscape" ? "w-[180px] ms-4" : layoutMode === "tablet" ? "w-[220px] ms-5" : "w-[500px] ms-6"}`}
        >
          <div
            className={`${layoutMode === "mobile" || layoutMode === "landscape" ? "w-[180px]" : layoutMode === "tablet" ? "w-[220px]" : "w-[500px]"} h-[3px] bg-[#ffffff33]`}
          >
            <div ref={progressRef} className="h-[3px] bg-[#ecad29]" />
          </div>
        </div>
        <div
          className={`overflow-hidden z-[60] relative ${layoutMode === "mobile" || layoutMode === "landscape" ? "w-[35px] h-[35px] ms-2" : layoutMode === "tablet" ? "w-[40px] h-[40px] ms-2" : "w-[50px] h-[50px]"}`}
        >
          {data.map((_, index) => (
            <div
              key={index}
              ref={(el) => {
                slideItemsRef.current[index] = el;
              }}
