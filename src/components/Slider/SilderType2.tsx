...
const SliderType2: React.FC<SliderType2Props> = ({ data }) => {
  ...
  return (
    ...
    <div
        key={index}
        ref={(el) => el && (navsRef.current[index] = el)}
        onClick={() => scrollToSection(index)}
        className={`h-full w-[52px] p-4 flex flex-col justify-between items-center absolute cursor-pointer z-[10] ${
        index === 0 ? '' : 'border-s-2 border-[#1c1c1c]'
        }`}
        style={{
        right: index === 0 ? 'auto' : `${52 * (data.length - 1 - index)}px`,
    ...
    >
        ...
        <span
            className="absolute left-1/2 top-1/2 whitespace-nowrap uppercase font-bold text-white"
            style={{
            transform: 'translate(-50%, -50%) rotate(-90deg)',
            textShadow: '1px 1px 4px rgba(0,0,0,0.6)', // optional for readability
            }}
        >
            {item.title}
        </span>
      ...
      {data.map((item, index) => (
        <div
          ...
          className="flex w-full h-full flex-shrink-0"
        >
          <div
            className="w-full h-full flex justify-center items-center flex-col gap-4 p-4 bg-cover bg-center ms-[50px]"
            style={{
              width: 'calc(100%)',
              backgroundImage: `url(${item.image})`,
            }}
